...
int main() {
    ...
    // -------------------------------------------------------
    {
        ...
        hft::OrderBook ob(N + 10);

        std::cout << "[1] Pure Insertion: " << N << " passive sell orders...\n";
        auto start = std::chrono::high_resolution_clock::now();

        for (uint64_t i = 1; i <= N; ++i) {
            ob.add_order(i, hft::OrderType::Limit, 1000 + (i % 1000), 10, hft::Side::Sell);
        }
    ...
    // -------------------------------------------------------
    {
        ...
        hft::OrderBook ob(SEED + MATCH + 10);
    ...
    // -------------------------------------------------------
    {
        ...
        // Seed
        for (uint64_t i = 1; i <= N; ++i) {
            ob.add_order(i, hft::OrderType::Limit, 1000 + (i % 500), 10, hft::Side::Sell);
        }
        ...
        for (uint64_t i = 1; i <= N; ++i) {
            ob.add_order(i, hft::OrderType::Cancel, 0, 0, hft::Side::Buy);
        }
    ...
    // -------------------------------------------------------
    {
        ...
        for (uint64_t i = 0; i < N; ++i) {
            ...
            if (op < 7) {
                // Insert passive
                ob.add_order(id++, hft::OrderType::Limit, 1000 + (i % 200), 10, hft::Side::Sell);
            } else if (op < 9) {
                // Aggressive match
                ob.add_order(id++, hft::OrderType::Limit, 1200, 10, hft::Side::Buy);
            } else {
                // Cancel a recent order
                uint64_t cancel_id = (id > 5) ? id - 5 : 1;
                ob.add_order(cancel_id, hft::OrderType::Cancel, 0, 0, hft::Side::Buy);
            }
        }
